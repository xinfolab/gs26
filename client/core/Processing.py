...
from PyQt5.QtWidgets import QLabel, QApplication, QPushButton, QDialog, QTableWidget, QTableWidgetItem, QMessageBox
...
class Ui_Processing(QDialog):
    ...
    def complete_scan(self, grt_flag, git_flag, gpt_flag, gft_flag):
        if(grt_flag and git_flag and gpt_flag and gft_flag):
            button_flag = QMessageBox.question(self, '완료', "보고서를 확인하시겠습니까?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
    ...
    def gfileThreadEventHandler(self, file_data):
        ...
        self.gft_flag = True
        self.complete_scan(self.grt_flag, self.git_flag, self.gpt_flag, self.gft_flag)
