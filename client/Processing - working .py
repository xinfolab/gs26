...
class Ui_Processing(QDialog):
    ...
    def processUI(self):
        ...
        self.tableWidget.setColumnWidth(3, self.tableWidget.width()/2.5)
        column_headers = ['RISK', 'FATH', 'Blank', 'Blabla']
    ...
    def gipThreadEventHandler(self, ip_data):
        ...
        for k in ip_data:
            ...
            self.tableWidget.setItem(i, 2, QTableWidgetItem("ip"))
            i = i + 1
            self.last_count = i
    ...
    def gregThreadEventHandler(self, reg_data):
        ...
        for reg_k in reg_data:
            ...
            self.tableWidget.setItem(i, 2, QTableWidgetItem("reg"))
            i = i + 1
            self.last_count = i
    ...
    def gprocThreadEventHandler(self, proc_data):
        ...
        for proc_k in proc_data.keys():
            ...
            self.tableWidget.setItem(i, 3, QTableWidgetItem("proc"))
            i = i + 1
            self.last_count = i
    ...
    def gfileThreadEventHandler(self, file_data):
        ...
        for k in file_data.keys():
            ...
            self.tableWidget.setItem(i, 3, QTableWidgetItem("file"))
            i = i + 1
            self.last_count = i
        #self.reportThread.count -= 1
    ...
    def P_btn_clicked(self):
        ...
        if self.gregThread.isRunning() :
            self.gregThread.terminate()
            self.gprocThread.terminate()
            self.gfileThread.terminate()
            print('reg ~ term')
            self.term_state = 1
        if self.gprocThread.isRunning() :
            self.gprocThread.terminate()
            self.gfileThread.terminate()
            print('proc ~ term')
            self.term_state = 2
        if self.gfileThread.isRunning() :
            self.gfileThread.terminate()
            print('file term')
            self.term_state = 3
    def C_btn_clicked(self):
        ...
        self.movie.start()
        if self.term_state == 1:
            self.gregThread.start()
            self.gprocThread.start()
            self.gfileThread.start()
        if self.term_state == 2:
            self.gprocThread.start()
            self.gfileThread.start()
        if self.term_state == 3:
            self.gfileThread.start()
    ...
    def stopThread(self):
        self.gipThread.terminate()
        self.gregThread.terminate()
        self.gprocThread.terminate()
        self.gfileThread.terminate()
        self.gipThread.wait()
        self.gregThread.wait()
        self.gprocThread.wait()
        self.gfileThread.wait()
        self.last_count = 0
        self.count = 0
